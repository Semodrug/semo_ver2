...
class ReviewService {
  ...
  final CollectionReference reportReviewCollection =
      FirebaseFirestore.instance.collection('ReportReview');

  Future<void> updateReviewData(String effect, String sideEffect,
      String effectText, String sideEffectText, num starRating) async {
  ...
  Future<void> tapToRate(num rating, String uid) async {
    return await reviewCollection
        .doc(documentId)
        .update({'starRating': rating, 'seqNum': documentId, 'uid': uid});
  }
...
//    return reviewsSnapshots;
    ...
    return reviewCollection.snapshots().map(_reviewListFromSnapshot);
  }
  ...
  Future<bool> findUserWroteReview(String seqNum, String user) async {
    final QuerySnapshot result = await reviewCollection
        .where("seqNum", isEqualTo: seqNum)
  ...
  static Future<QuerySnapshot> newgetReviews(int limit) async {
    final refReviews = FirebaseFirestore.instance
        .collection('Reviews')
...
//        .orderBy("registrationDate", "asc")
  ...
  static Future<QuerySnapshot> getReviewData(
    int limit, {
